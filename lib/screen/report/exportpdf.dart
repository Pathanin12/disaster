        ...
        child: pw.Align(
          ...
          child: pw.Column(
            ...
            children: [
              ...
              pw.Text((startDate==endDate)?'${startDate.day} ${mountList[startDate.month-1]} ${startDate.year}':'${startDate.day} ${mountList[startDate.month-1]} ${startDate.year} - ${endDate.day} ${mountList[endDate.month-1]} ${endDate.year} ',
                  style: pw.TextStyle(font: ttf, fontSize: 12)),
            ],
          ),
        ),
...
Future<void> createPDF(GetAllEventModel allEven,DateTime startDate,DateTime endDate,String type,String level,String province) async {
            ...
            for (var row in allEven.eventList!)
              pw.TableRow(
                children: [
                  ...
                  _buildTableCell(category[row.disasterType!], ttf),
                  _buildTableCell('${DateTime.parse(row.datetime!).day} ${mountAbbreviation[DateTime.parse(row.datetime!).month]} ${DateTime.parse(row.datetime!).year}', ttf),
                  _buildTableCell(row.responsibleAgency!, ttf),
                  _buildTableCell('${row.latitude},${row.longitude}', ttf),
                  _buildTableCell(status[row.statusRelatedAgency!], ttf),
                  _buildTableCell(row.relatedAgency!, ttf),
